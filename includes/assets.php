 ...
 * @return  array|null
 */
function auto_flickr_importer_get_asset_meta( string $asset_path, ?array $extra_dependencies = null ): ?array {
	if ( ! file_exists( $asset_path ) || ! str_starts_with( $asset_path, AUTO_FLICKR_IMPORTER_PATH ) ) {
		return null;
	}
	...
	if ( file_exists( $asset_path_info['dirname'] . '/' . $asset_path_info['filename'] . '.asset.php' ) ) {
		...
		if ( false === $asset_meta['version'] ) { // Safeguard against filemtime() returning false.
			$asset_meta['version'] = AUTO_FLICKR_IMPORTER_METADATA['Version'];
		}
	}
